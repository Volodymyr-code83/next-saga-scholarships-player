...
interface ServiceDescImageModalProps {
  isOpen: boolean;
  imageUrl: string;
  onClose: () => void;
...
const ServiceDescImageModal = ({
  isOpen,
  imageUrl,
  onClose,
  onSubmit,
}: ServiceDescImageModalProps) => {
      ...
      <form onSubmit={handleSubmit(onFormSubmit)}>
        ...
        >
          <div className="flex w-full flex-col items-start justify-center gap-[22px]">
            <div className="relative mx-auto flex h-full max-h-[597px] min-h-[597px] w-full max-w-[926px] items-center justify-center  rounded-[20px] bg-[#9D9595]">
              {image && (
                <Image
                  src={image}
                  width={926}
...
export default ServiceDescImageModal;
